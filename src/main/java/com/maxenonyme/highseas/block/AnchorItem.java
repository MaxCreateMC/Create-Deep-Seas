...
public class AnchorItem extends BlockItem {
    private static final int CREATE_YELLOW = 0xEBC255;

    public AnchorItem(Block block, Properties properties) {
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltip, TooltipFlag flag) {
        if (Screen.hasShiftDown()) {
            ...
            addLines(tooltip, "item.create_high_seas.anchor.tooltip.summary", CREATE_YELLOW);
            addLines(tooltip, "item.create_high_seas.anchor.tooltip.behaviour1", CREATE_YELLOW);
            
            tooltip.add(Component.empty());
            addLines(tooltip, "item.create_high_seas.anchor.tooltip.condition1", ChatFormatting.GRAY.getColor());
            addLines(tooltip, "item.create_high_seas.anchor.tooltip.behaviour2", CREATE_YELLOW);
        } else {
