...
public class RudderItem extends BlockItem {
    private static final int CREATE_YELLOW = 0xEBC255;

    public RudderItem(Block block, Properties properties) {
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltip, TooltipFlag flag) {
        if (Screen.hasShiftDown()) {
            ...
            addLines(tooltip, "item.create_high_seas.rudder.tooltip.summary", CREATE_YELLOW);
            addLines(tooltip, "item.create_high_seas.rudder.tooltip.behaviour1", CREATE_YELLOW);
            addLines(tooltip, "item.create_high_seas.rudder.tooltip.behaviour2", CREATE_YELLOW);
        } else {
